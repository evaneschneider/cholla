...
{
  Real total_energy, thermal_energy;

  G.Analysis_Functions(&total_energy, &thermal_energy);

  #ifdef MPI_CHOLLA
  MPI_Barrier(world);
  ReduceRealSum(total_energy);
  ReduceRealSum(thermal_energy);
  if (procID == 0) {
  #endif 
  ...
  fprintf(out, "%8.3f %8.5e %8.5e\n", G.H.t, total_energy, thermal_energy);
