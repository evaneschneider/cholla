                                              ...
                                              Real gamma, int n_fields)
{
  ...
  if (xid > n_ghost-1 && xid < nx-n_ghost && yid > n_ghost-1 && yid < ny-n_ghost && zid > n_ghost-1 && zid < nz-n_ghost)
  {
    ...
    #endif    
/*
    if (dev_conserved[5*n_cells+id]/dev_conserved[id] > 1.0) {
      printf("%3d %3d %3d %e %e %e\n", xid+x_off, yid+y_off, zid+z_off, dev_conserved[5*n_cells+id]/dev_conserved[5*n_cells+id], dev_conserved[id], dev_conserved[4*n_cells+id]);
    }
*/
    /*
